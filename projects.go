...
func (s *ProjectService) handleCreateProject(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		WriteJSON(w, http.StatusInternalServerError, ErrorResponse{Error: "Error checking project existence"})
		return
	}

	if prjExist {
		WriteJSON(w, http.StatusConflict, ErrorResponse{Error: "Project with this name already exists"})
		return
	}
